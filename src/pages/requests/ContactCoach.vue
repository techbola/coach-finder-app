<template>
  <form @submit.prevent='submitForm'>
    <div class='form-control'>
      ...
      <p class='errors' v-if='!email.isValid'>Email address must not be empty</p>
    </div>
    <div class='form-control'>
      <label for='message'>Message</label>
      <textarea id='message' rows='5' v-model.trim='message.val'></textarea>
      <p class='errors' v-if='!message.isValid'>Message must not be empty</p>
    </div>
  ...
  methods: {
    ...
    validateForm() {
      ...
      if (this.email.val === '' || !this.email.val.includes('@')) {
        ...
        this.formIsValid.isValid = false
      }
      if (this.message.val === '') {
